...
public class MTCNN {
    ...
    private  int PNetForward(Bitmap bitmap, float [][]PNetOutProb, float[][][]PNetOutBias){
        ...
        Utils_MTCNN.flip_diag(PNetIn,h,w,3); //沿着对角线翻转
        ...
        //【写法一】先翻转，后转为2/3维数组
        Utils_MTCNN.flip_diag(PNetOutP,PNetOutSizeW,PNetOutSizeH,2);
        Utils_MTCNN.flip_diag(PNetOutB,PNetOutSizeW,PNetOutSizeH,4);
        Utils_MTCNN.expand(PNetOutB,PNetOutBias);
        Utils_MTCNN.expandProb(PNetOutP,PNetOutProb);
        /*
    ...
    private Vector<Box> PNet(Bitmap bitmap, int minSize){
        ...
        return Utils_MTCNN.updateBoxes(totalBoxes);
    }
    ...
    private Vector<Box> RNet(Bitmap bitmap, Vector<Box> boxes){
        ...
        for (int i=0;i<num;i++){
            ...
            Utils_MTCNN.flip_diag(curCrop,24,24,3);
        ...
        return Utils_MTCNN.updateBoxes(boxes);
    }
    /*
    ...
    private Vector<Box> ONet(Bitmap bitmap, Vector<Box> boxes){
        ...
        for (int i=0;i<num;i++){
            ...
            Utils_MTCNN.flip_diag(curCrop,48,48,3);
